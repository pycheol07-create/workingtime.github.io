...
export function renderCostAnalysisConfig(config) {
    ...
    if (container) {
        ...
        if (allTasks.size === 0) {
             container.innerHTML = '<p class="text-xs text-gray-400 col-span-full text-center py-4">등록된 업무가 없습니다.</p>';
        } else {
            ...
            <div class="flex flex-col gap-3 mb-4 p-4 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50/50 dark:bg-gray-900/30 shadow-sm member-item transition-colors">
                ...
                <div class="flex flex-wrap md:flex-nowrap justify-between items-start gap-4">
                    <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                        ...
                        <div class="flex flex-col">
                            ...
                            <input type="text" value="${member}" class="member-name w-24 p-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded-md text-sm font-bold dark:text-white" placeholder="이름">
                        </div>
                        
                        <div class="flex flex-col">
                            <label class="text-[10px] text-gray-500 dark:text-gray-400 mb-1">이메일</label>
                            <input type="email" value="${memberEmail}" class="member-email w-48 p-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded-md text-sm dark:text-white" placeholder="email">
                        </div>

                        <div class="flex flex-col">
                            <label class="text-[10px] text-gray-500 dark:text-gray-400 mb-1">시급</label>
                            <input type="number" value="${memberWages[member] || 0}" class="member-wage w-24 p-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded-md text-sm dark:text-white" placeholder="시급">
                        </div>
                ...
                <div class="flex flex-wrap items-center gap-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="date" value="${joinDate}" class="member-join-date w-32 p-1.5 border border-blue-200 dark:border-blue-800 bg-white dark:bg-gray-800 rounded text-xs dark:text-gray-200">
                    </div>
                    <div class="flex flex-col">
                        <label class="text-[9px] text-blue-600 dark:text-blue-400 mb-1">총연차(일)</label>
                        <input type="number" value="${totalLeave}" class="member-total-leave w-16 p-1.5 border border-blue-200 dark:border-blue-800 bg-white dark:bg-gray-800 rounded text-xs text-center dark:text-gray-200" min="0">
                    </div>
                    
                    <div class="hidden md:block w-px h-8 bg-gray-300 dark:bg-gray-600 mx-2"></div>

                    <div class="flex flex-col">
                        <label class="text-[9px] text-gray-500 dark:text-gray-400 mb-1 font-bold" title="이 날짜 이후의 연차만 차감 계산됩니다.">적용 시작일</label>
                        <input type="date" value="${leaveResetDate}" class="member-leave-reset-date w-32 p-1.5 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded text-xs font-bold text-gray-700 dark:text-gray-200">
                    </div>
                    <div class="flex flex-col">
                        <label class="text-[9px] text-red-500 dark:text-red-400 mb-1 font-bold">사용 만료일</label>
                        <input type="date" value="${expirationDate}" class="member-leave-expiration-date w-32 p-1.5 border border-red-200 dark:border-red-800 bg-white dark:bg-gray-800 rounded text-xs text-red-700 dark:text-red-400">
                    </div>
                </div>
            </div>
            `;
        ...
        groupEl.innerHTML = `
            <div class="flex justify-between items-center mb-5 pb-3 border-b border-gray-100 dark:border-gray-700">
                <div class="flex items-center gap-2">
                    <span class="drag-handle mr-2 cursor-move text-lg" draggable="true">☰</span> 
                    <input type="text" value="${group.name}" class="text-lg font-extrabold text-gray-800 dark:text-white team-group-name w-auto bg-transparent border-b border-transparent hover:border-gray-300 dark:hover:border-gray-600 focus:border-blue-500 p-1">
                </div>
...
export function renderDashboardItemsConfig(itemIds, fullConfig) {
    ...
    itemIds.forEach((id, index) => {
        ...
        let itemHtml = `
            <span class="drag-handle text-gray-400 cursor-move" draggable="true">☰</span>
...
export function renderTaskGroups(taskGroups) {
    ...
    (taskGroups || []).forEach((group, index) => {
        ...
        groupEl.innerHTML = `
             <div class="flex justify-between items-center mb-5 pb-3 border-b border-gray-100 dark:border-gray-700">
                <div class="flex items-center gap-2"> 
                   <span class="drag-handle cursor-move text-lg" draggable="true">☰</span>
                   <input type="text" value="${group.name}" class="text-lg font-extrabold text-gray-800 dark:text-white task-group-name w-auto bg-transparent border-b border-transparent hover:border-gray-300 dark:hover:border-gray-600 focus:border-blue-500 p-1">
                 </div>
