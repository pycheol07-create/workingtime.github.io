...
export const renderWeeklyHistory = (allHistoryData, appConfig) => {
    ...
    try {
        ...
        // ✅ [수정] map 콜백에서 index를 사용해 prevData 전달 및 CSV 버튼 추가
        view.innerHTML = sortedWeeks.map((weekKey, index) => {
            ...
            `;
            // ✅ [수정] 마지막 인자로 버튼 HTML 전달
            return renderSummaryView('weekly', currentData, weekKey, combinedWageMap, prevData, csvButtonHtml); 
        }).join('');
...
export const renderMonthlyHistory = (allHistoryData, appConfig) => {
    ...
    try {
        ...
        view.innerHTML = sortedMonths.map((monthKey, index) => {
            ...
            `;
            // ✅ [수정] 마지막 인자로 버튼 HTML 전달
            return renderSummaryView('monthly', currentData, monthKey, combinedWageMap, prevData, csvButtonHtml); 
        }).join('');
...
// ✅ [수정] 주별/월별 근태 요약 렌더링 공통 헬퍼 함수 (HTML 문자열 반환)
const renderAggregatedAttendanceSummary = (data, periodKey, csvButtonHtml = '') => {
