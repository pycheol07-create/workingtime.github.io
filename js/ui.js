...
export const renderAnnouncement = (appConfig) => {
    ...
    const announcementText = appConfig.workAnnouncement || '';

    if (!announcementText) {
        contentDiv.innerHTML = `<p class="text-gray-500">등록된 공지사항이 없습니다.</p>`;
    } else {
        // textarea의 줄바꿈(\n)을 HTML <br>로 치환하여 표시
        // 보안을 위해 텍스트만 표시 (HTML 태그 직접 삽입 방지)
        const escapedText = announcementText
            .replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
            .replace(/"/g, "&quot;")
