...
// === ui.js (파일 맨 끝에 새 함수 2개 추가) ===

/**
 * ✅ [추가] 실시간 근태 현황 리스트를 렌더링합니다.
 */
export const renderAttendanceSummaryList = (appState) => {
    const container = document.getElementById('realtime-attendance-list');
    if (!container) return;
    ...
    // 외출 복귀한 사람 제외
    const onLeaveStatus = combinedOnLeaveMembers
        .filter(item => !(item.type === '외출' && item.endTime))
        .sort((a, b) => a.member.localeCompare(b.member)); // 이름순 정렬

    if (onLeaveStatus.length === 0) {
        container.innerHTML = `<p class="text-gray-500 text-sm">현재 근태 현황이 없습니다.</p>`;
        return;
    }

    let html = '';
    onLeaveStatus.forEach(item => {
        let detailText = '';
        if (item.startTime) { // 외출, 조퇴
            detailText = `(${formatTimeTo24H(item.startTime)} ~ ${item.endTime ? formatTimeTo24H(item.endTime) : '...'})`;
        } else if (item.startDate) { // 연차, 출장, 결근
            detailText = `(${item.startDate.substring(5)}${item.endDate && item.endDate !== item.startDate ? ` ~ ${item.endDate.substring(5)}` : ''})`;
        }

        html += `
            <div class="flex justify-between items-center text-sm p-1.5 rounded-md hover:bg-gray-100">
                <span class="font-semibold text-gray-800">${item.member}</span>
                <span class="font-medium text-red-600">${item.type}</span>
 ...
 * (단순 텍스트, 줄바꿈만 적용)
 */
export const renderNoticeBoard = (noticeText) => {
    const container = document.getElementById('notice-board-content');
    if (!container) return;

    if (!noticeText || noticeText.trim() === '') {
        container.innerHTML = `<p class="text-gray-500 text-sm">등록된 공지사항이 없습니다.</p>`;
        return;
    }
    
    // XSS 방지를 위해 텍스트로 설정 (whitespace-pre-wrap 클래스가 줄바꿈 처리)
    container.textContent = noticeText;
};