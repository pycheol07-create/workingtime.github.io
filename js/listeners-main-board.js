...
import { render } from './app.js?v=202609080922';
import { showToast, formatTimeTo24H } from './utils.js?v=202609080922';
import { renderTeamSelectionModalContent, renderLeaveTypeModalOptions } from './ui.js?v=202609080922';
import {
...
} from './app-logic.js?v=202609080922';

import { renderTodayInspectionList, initializeInspectionSession } from './inspection-logic.js?v=202609080922';
import { searchTotalInspection } from './total-inspection-logic.js?v=202609080922';
...
export function setupMainBoardListeners() {
    ...
    // 💡 분리된 두 보드(업무현황, 팀원현황)에 공통으로 적용할 클릭 핸들러
    ...
    // 💡 분리된 두 보드(업무 현황, 팀원 현황)에 각각 리스너 연결
    const taskBoard = document.getElementById('task-status-board');
    if (taskBoard) taskBoard.addEventListener('click', handleBoardClick);

    const memberBoard = document.getElementById('member-status-board');
    if (memberBoard) memberBoard.addEventListener('click', handleBoardClick);
