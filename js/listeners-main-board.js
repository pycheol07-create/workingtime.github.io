    ...
    if (DOM.teamStatusBoard) {
        DOM.teamStatusBoard.addEventListener('click', (e) => {
            ...
            if (stopGroupButton) {
                ...
                if (DOM.stopGroupConfirmModal) DOM.stopGroupConfirmModal.classList.remove('hidden');
                return;
            }

            const pauseGroupButton = e.target.closest('.pause-work-group-btn');
            if (pauseGroupButton) { pauseWorkByTask(pauseGroupButton.dataset.task); return; }
            
            const resumeGroupButton = e.target.closest('.resume-work-group-btn');
            if (resumeGroupButton) { resumeWorkByTask(resumeGroupButton.dataset.task); return; }
            
            const individualPauseBtn = e.target.closest('[data-action="pause-individual"]');
            if (individualPauseBtn) { pauseWorkIndividual(individualPauseBtn.dataset.recordId); return; }
            
            const individualResumeBtn = e.target.closest('[data-action="resume-individual"]');
            if (individualResumeBtn) { resumeWorkIndividual(individualResumeBtn.dataset.recordId); return; }
            ...
            if (individualStopBtn) {
                ...
                if (DOM.stopIndividualConfirmMessage && record) DOM.stopIndividualConfirmMessage.textContent = `${record.member}님의 '${record.task}' 업무를 종료하시겠습니까?`;
