// === app.js (파일 분리 수정 1안, 핵심 로직 및 상태 관리 담당) ===
...
// ❗️ [삭제] app-history.js 및 app-events.js 임포트 구문 제거
...
// ❗️ (normalizeName, debounce 등은 utils.js에서 가져옴)
...
export async function startAppAfterLogin(user) {
  ...
  try {
    ...
    // ❗️ [삭제] setupDynamicEventListeners(); 호출 제거
  ...
  unsubscribeConfig = onSnapshot(configDocRef, (docSnap) => {
    if (docSnap.exists()) {
      ...
      if (addAttendanceMemberDatalist) {
        // ❗️ app-history.js의 함수를 직접 호출할 수 없으므로,
        // ❗️ app.js에서 이 DOM을 직접 업데이트하거나, app-history.js가 이 스냅샷을 수신해야 함.
        // ❗️ 여기서는 일단 app.js에 해당 함수를 다시 가져옵니다. (임시)
        renderAttendanceAddModalDatalists_local(appConfig);
      }
  ...
  // ❗️ config 스냅샷을 위한 임시 로컬 함수
  const renderAttendanceAddModalDatalists_local = (appConfig) => {
...
async function main() {
  ...
  // ❗️ 드래그 기능 활성화 코드는 app-events.js로 이동했습니다.
}
